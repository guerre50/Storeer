...
define(['module', 'normalize'], function(module, normalize) {
  ...
  cssAPI.baseUrl = config.baseUrl || require.toUrl('');
  ...
  cssAPI.load = function(cssId, req, load, config, parse) {
    ...
    fileUrl = baseUrl + (baseUrl.charAt((baseUrl.length - 1)) == '/'? '': '/') + resourceId;
